...
*Figure 1: v0.5.2 Hardware Analysis & Normalization Pipeline*

## 🌍 Next Generation: Live Grid API Integration (In Development)

EcoTrace is entering the "Scientific Integrity" era. We are currently working on integrating real-time carbon intensity data from global energy grids into the core engine.

**Current Focus:**
- **🛰️ Live Carbon Intensity:** Moving from static regional constants to real-time API data (Electricity Maps integration).
- **📉 Carbon-Aware Scheduling:** Actionable advice on when to run heavy workloads based on grid cleanliness.
- **🛡️ CI/CD Carbon Gates:** Automating green code standards in development pipelines.

> [!NOTE]
> **Active Development:** Live Grid API integration is currently being implemented. Stay tuned for the v0.6.0 release!
...
*Figure 2: EcoTrace Logic in Action*
...
EcoTrace v0.5.0 integrates **Google Gemini AI** to transform raw metrics into actionable optimization advice. Instead of static rules, it analyzes your hardware, region, and function semantics to suggest "Green Coding" improvements.
...
- **Vectorization Suggestions:** "Your i7-13700H supports AVX-512; use NumPy for this loop to save 15% energy."
- **Architecture Tuning:** "This function is I/O bound; switching to `asyncio` could reduce idle CPU carbon by 20%."
- **Library Swaps:** "Consider `lxml` instead of `xml.etree` for this specific workload to improve energy efficiency."
...
> [!NOTE]
> **Opt-in Feature:** Gemini AI Insights is an optional, developer-driven feature. It is only activated when a valid `GEMINI_API_KEY` is provided. If no key is detected, EcoTrace continues to operate with its high-precision local monitoring and static rule-based insights.
...
| **AI Insights (Gemini)** | ✅ **Hardware-Aware** | ❌ | ❌ |

...
*Real-time carbon tracking · 38-country live grid data · AI-powered insights · Zero-config deployment*
...
[![38 Countries](https://img.shields.io/badge/🗺️_Coverage-38_Countries-4169E1?style=flat-square)](https://github.com/Zwony/ecotrace)
...
*EcoTrace v6.0 — Sustainability OS Pipeline*
...
## 🆕 What's New in v6.0 — Sustainability OS

EcoTrace v6.0 transforms from a measurement tool into a **Sustainability Operating System** with two game-changing features:
...
```
[EcoTrace] 🌱 Yeni bir sürüm mevcut! (v6.0.0 → v6.1.0)
[EcoTrace] Güncellemek ister misiniz? (y/n):
```
...
| Feature | **EcoTrace v6.0** | CodeCarbon | CarbonTracker |
|---|:---:|:---:|:---:|
| **API Style** | ✅ One-line `@track` decorator | ⚠️ Context manager only | ❌ Manual start/stop |
| **Granularity** | ✅ Per-function | ⚠️ Session-level | ⚠️ Epoch-level |
| **Live Grid API** | ✅ Electricity Maps (38 zones) | ❌ Static only | ❌ Static only |
| **Auto-Update** | ✅ Interactive PyPI check | ❌ | ❌ |
| **Async Support** | ✅ Auto-detected | ❌ | ❌ |
| **Continuous Sampling** | ✅ 50ms daemon threads | ❌ Point-in-time | ❌ Point-in-time |
| **GPU Support** | ✅ NVIDIA + AMD + Intel | ⚠️ NVIDIA only | ⚠️ NVIDIA only |
| **Process Isolation** | ✅ `psutil.Process()` | ❌ System-wide | ❌ System-wide |
| **PDF Reports** | ✅ Built-in with charts | ❌ | ❌ |
| **AI Insights** | ✅ Gemini-powered | ❌ | ❌ |
| **Function Comparison** | ✅ `eco.compare()` | ❌ | ❌ |
| **CPU TDP Database** | ✅ 1,800+ models (Boavizta) | ⚠️ ~1,000 models | ❌ Manual config |
| **Crash-Proof** | ✅ Result always returned | ❌ Raises exceptions | ❌ Raises exceptions |
...
EcoTrace v6.0 fetches **real-time carbon intensity** from the Electricity Maps API, replacing static constants with live grid data:

```
┌─────────────────────────────────────────────────┐
│ Tier 1: Memory Cache                            │
│   → Valid cached value within 1 hour? → Use it  │
├─────────────────────────────────────────────────┤
│ Tier 2: Live API Fetch                          │
│   → Query Electricity Maps → Success? → Cache   │
├─────────────────────────────────────────────────┤
│ Tier 3: Static Fallback                         │
│   → constants.json → Always works               │
└─────────────────────────────────────────────────┘
```
...
EcoTrace is engineered for **production deployment** with multiple layers of defensive design:

| Safety Feature | Implementation |
|---|---|
| **GPU Fallback Chain** | NVIDIA (`pynvml`) → AMD/Intel (`WMI`) → graceful `None` — never crashes |
| **Live Grid Fallback** | API → Memory Cache → Static `constants.json` — always resolves |
| **Auto-Update Safety** | Network error? Timeout? CI/CD mode? → Silently skipped |
| **Thread-Safe Sampling** | `threading.Lock` guards all buffers; `deque(maxlen)` prevents memory leaks |
| **Process Isolation** | `psutil.Process()` scopes measurement — no cross-process noise |
| **TDP Fallback** | Unknown CPUs default to 65W; GPUs use vendor estimates (Intel: 15W, AMD: 75W) |
| **Crash-Proof Decorators** | Measurement fails? Function result is **always** returned |
| **Clean Shutdown** | `__del__` + daemon threads ensure monitoring stops on exit |

---

## 🗺️ Global Coverage — 38 Countries

EcoTrace supports **38 countries** with both static carbon intensity values and live Electricity Maps zone mappings:
...
*Static values from IEA 2022 global averages. With Live Grid API enabled, values update in real-time.*
...
|---|---|---|
| **v6.0** | 🌍 Live Grid API — Real-time carbon intensity (Electricity Maps) | ✅ Released |
| **v6.0** | 🌱 Auto-Update — Interactive PyPI version check | ✅ Released |
| **v6.1** | 📉 Carbon-Aware Scheduling — Optimal run-time suggestions based on grid | 🔨 In Progress |
| **v6.1** | 🛡️ CI/CD Carbon Gates — GitHub Actions integration for green thresholds | 🔨 In Progress |
| **v7.0** | ☁️ Cloud Dashboard — Real-time emissions visualization web UI | 🔜 Planned |
| **v7.0** | 📊 Automated ESG Reports — CSRD and GHG Protocol-compliant exports | 🔜 Planned |
| **v7.x** | 🔗 AWS / Azure / GCP — Cloud resource carbon attribution | 🔜 Planned |
| **v8.0** | 🏢 Team Carbon Budgets — Multi-project rollup with alerting | 🔜 Planned |

...
**Type:** Production/Stable Release
...
The `carbon_limit` parameter now actively enforces a carbon budget with two-tier alerts:
...
An idle CPU baseline is measured at startup and subtracted from all subsequent readings. Only the energy directly attributable to user code is reported.

**Session Summary**
An `atexit` hook prints a formatted summary when the process exits:
...
**Carbon Equivalences**
The `equivalence(gco2)` method converts abstract gCO2 values into human-readable comparisons:
...
**CI/CD Carbon Gate**
`ecotrace gate --budget <gCO2>` returns exit code 1 if accumulated emissions exceed the budget. Designed for GitHub Actions and GitLab CI.
...
Programmatic access to the remaining carbon budget for external consumers such as IDE extensions and dashboards.

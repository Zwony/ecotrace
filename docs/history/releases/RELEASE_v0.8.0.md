...
**Release Date:** 2026-04-24 **Type:** Minor Release
...
**Unified CLI Tool** The `ecotrace` command-line interface is now available with the following subcommands:
...
**JSON Export API** `EcoTrace.export_json(path)` exports the full session emission log to a structured JSON file. Used by the VS Code extension to render real-time charts.

**`python -m ecotrace` Support** The package now includes a `__main__.py` module enabling direct invocation:
